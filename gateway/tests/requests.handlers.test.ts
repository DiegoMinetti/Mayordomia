...
const AUTH_PERMISSIONS = [
  ...
  { id: 'rp_bbb', organizationId: 'org_123456', roleId: 'role_aaa', permission: 'request.approve.area' },
  { id: 'rp_ccc', organizationId: 'org_123456', roleId: 'role_aaa', permission: 'request.approve.general' },
];
...
function fakeSheets(options: FakeSheetsOptions = {}): SheetsClient {
...
describe('requests handlers', () => {
  ...
  it('requests.list returns org-scoped requests sorted desc by createdAt', async () => {
    const requestsRows: FakeRow[] = [
      { id: 'req_aaa', organizationId: 'org_123456', type: 'OTHER', status: 'PENDING', source: 'INTERNAL', requesterName: 'A', description: 'a', createdAt: '2026-01-01T00:00:00.000Z', version: 1 },
      { id: 'req_bbb', organizationId: 'org_123456', type: 'OTHER', status: 'PENDING', source: 'INTERNAL', requesterName: 'B', description: 'b', createdAt: '2026-02-01T00:00:00.000Z', version: 1 },
      { id: 'req_ccc', organizationId: 'org_other_xxx', type: 'OTHER', status: 'PENDING', source: 'INTERNAL', requesterName: 'C', description: 'c', createdAt: '2026-03-01T00:00:00.000Z', version: 1 },
    ];
    const sheets = fakeSheets({ rows: { Requests: requestsRows } });
    const audit = makeAuditService(sheets, silentLogger);
    register('requests.list', { auth: true, permission: 'request.review' }, () => ({ requests: [] }));
    ...
    const handlers = makeRequestsHandlers({ sheets, audit });
    register('requests.list', { auth: true, permission: 'request.review' }, (p, ctx) => handlers.list(p, ctx));
    const result = await dispatch(
      { action: 'requests.list', organizationId: 'org_123456', auth: { accessToken: 'tok' } },
      { sheets, audit },
    );
    expect((result as { requests: Array<{ id: string }> }).requests.map((r) => r.id)).toEqual(['req_bbb', 'req_aaa']);
  });

  it('requests.approve enforces optimistic concurrency and audits', async () => {
    const requestRow = {
      id: 'req_aaa', organizationId: 'org_123456', type: 'OTHER', status: 'PENDING',
      source: 'INTERNAL', requesterName: 'A', description: 'a',
      createdAt: '2026-01-01T00:00:00.000Z', version: 3,
    };
    const approvalRow = {
      id: 'appr_aaa', organizationId: 'org_123456', requestId: 'req_aaa',
      scope: 'AREA', areaId: undefined, status: 'PENDING',
    ...
    await expect(
      dispatch(
        { action: 'requests.approve', organizationId: 'org_123456', auth: { accessToken: 'tok' }, payload: { id: 'req_aaa', scope: 'AREA', expectedVersion: 1, comment: 'ok' } },
        { sheets, audit },
      ),
    ...
    const result = await dispatch(
      { action: 'requests.approve', organizationId: 'org_123456', auth: { accessToken: 'tok' }, payload: { id: 'req_aaa', scope: 'AREA', expectedVersion: 3, comment: 'ok' } },
      { sheets, audit },
    );
  ...
  it('requests.get builds a timeline and includes approvals', async () => {
    ...
    const approvalRow = {
      id: 'appr_aaa', organizationId: 'org_123456', requestId: 'req_aaa',
      scope: 'GENERAL', status: 'APPROVED', reviewedBy: 'u1',
      reviewedAt: '2026-01-02T00:00:00.000Z', comment: 'lgtm',
    ...
    const { makeRequestsHandlers } = await import('../src/requests/handlers.js');
    const handlers = makeRequestsHandlers({ sheets, audit });
    register('requests.get', { auth: true, permission: 'request.review' }, (p, ctx) => handlers.get(p, ctx));
    const result = await dispatch(
      { action: 'requests.get', organizationId: 'org_123456', auth: { accessToken: 'tok' }, payload: { id: 'req_aaa' } },
      { sheets, audit },
    );
    const req = (result as { request: { approvals: unknown[]; timeline: Array<{ kind: string }> } }).request;
  ...
  it('requests.reject transitions to REJECTED and audits', async () => {
    const requestRow = {
      id: 'req_aaa', organizationId: 'org_123456', type: 'OTHER', status: 'PENDING_AREA_APPROVAL',
      source: 'INTERNAL', requesterName: 'A', description: 'a',
      createdAt: '2026-01-01T00:00:00.000Z', version: 5,
    };
    const approvalRow = {
      id: 'appr_aaa', organizationId: 'org_123456', requestId: 'req_aaa',
      scope: 'AREA', status: 'PENDING', createdAt: '2026-01-01T00:00:00.000Z',
      version: 1,
    };
    const sheets = fakeSheets({
      rows: { Requests: [requestRow], RequestApprovals: [approvalRow] },
    ...
    const result = await dispatch(
      { action: 'requests.reject', organizationId: 'org_123456', auth: { accessToken: 'tok' }, payload: { id: 'req_aaa', scope: 'AREA', expectedVersion: 5, comment: 'no' } },
      { sheets, audit },
    );
  ...
  it('requests.approve on a closed request throws REQUEST_LOCKED', async () => {
    const requestRow = {
      id: 'req_aaa', organizationId: 'org_123456', type: 'OTHER', status: 'APPROVED',
      source: 'INTERNAL', requesterName: 'A', description: 'a',
      createdAt: '2026-01-01T00:00:00.000Z', version: 1,
    };
    const sheets = fakeSheets({
    ...
    await expect(
      dispatch(
        { action: 'requests.approve', organizationId: 'org_123456', auth: { accessToken: 'tok' }, payload: { id: 'req_aaa', scope: 'AREA', expectedVersion: 1, comment: '' } },
        { sheets, audit },
      ),
  ...
  it('rejects ApiException on bad inputs', async () => {
    ...
    await expect(
      dispatch(
        { action: 'requests.approve', organizationId: 'org_123456', auth: { accessToken: 'tok' }, payload: { id: 'req_aaa', scope: 'NOPE', expectedVersion: 1, comment: '' } },
        { sheets, audit },
      ),
